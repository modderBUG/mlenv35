...
# 引用自定义模块
import MOOC.CNN.vgg16.Vgg16 as vgg16
import MOOC.CNN.vgg16.utils as utils
from MOOC.CNN.vgg16.Nclasses import labels

testNum = 1  # eval(input("input the number of test pictures:"))
for i in range(testNum):
    # img_path = raw_input('Input the path and image name:')
    img_path = 'pic/802.jpg'  # input('Input the path and image name:')
    while not os.path.exists(img_path):
        img_path = input('Input the path and image name:')
    # 对待测试图像出预处理操作1,224,224
