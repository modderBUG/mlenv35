    ...
    决策树：
        基本算法原理：
            核心思想：相似的输入必会产生相似的输出。例如预测某人薪资：
                    ...
                    为了提高搜索效率，使用树形数据结构处理样本数据：先按照年龄分三棵子树，再按照学历往下分，知直到所有特征都分完
                    这样便得到叶级子表，叶级子表中便保存了所有特征值完全相同的样本。

            首先从训练样本矩阵中选择第一个特征进行子表划分，使每个子表中该特征的值全部相同，
            然后再在每个子表中选择下一个特征按照同样的规则继续划分更小的子表，不断重复直到所有的特征全部使用完为止，
            此时便得到叶级子表，其中所有样本的特征值全部相同。对于待预测样本，根据其每一个特征的值，选择对应的子表，
            逐一匹配，直到找到与之完全匹配的叶级子表，用该子表中样本的输出，通过平均(回归)或者投票(分类)为待预测样本提供输出。
